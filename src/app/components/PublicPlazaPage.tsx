...
import { ChevronLeft, Users, Plane, Moon, FileText, Star, Check, Clock } from 'lucide-react';
...
interface Neighbor { tag: string; field: string; alias: string; blurb: string; sim: number }
      ...
      <div className="px-3 pt-3 shrink-0">
        <div className="border-2 border-black bg-white p-3 shadow-[2px_2px_0_rgba(0,0,0,0.85)]">
          <div className="flex items-center gap-2 mb-1.5">
            <Plane className="w-4 h-4" strokeWidth={2.5} style={{ color: ACCENT }} />
        ...
        {phase === 'day' && neighbors.map((n, i) => (
          <motion.div key={n.tag} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
            ...
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 shrink-0 border-2 border-black flex items-center justify-center font-pixel text-[9px] text-white" style={{ background: ACCENT }}>
                {n.alias.slice(0, 1)}
              </div>
              <div className="min-w-0 flex-1">
                <div className="text-[12px] font-bold truncate">{n.alias}</div>
                <div className="text-[10px] text-black/55 leading-tight truncate">{n.blurb}</div>
              </div>
              <div className="shrink-0 flex items-center gap-1 font-pixel text-[8px]" style={{ color: ACCENT }}>
            ...
            {report.map((n, i) => {
              ...
              return (
                <motion.div key={n.tag} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  ...
                  <div className="text-[11px] text-black/70 leading-snug mb-2">
                    你的特使今天和 ta 聊得来{n.blurb ? `，${n.blurb}` : ''}。要不要再进一步了解？
                  </div>
                  {d ? (
