...
music-curator 是「流水线型」子 agent 的代表，也是现阶段做得最透的一个 agent。它不靠你手选歌单，而是**读懂你此刻的上下文，自主把一份歌单建出来**。承载这条能力的是 `open-dj-director`：
...
|---|---|---|---|
| **music-curator** | 曲库：621 首按 **地域 / 城市 / 歌手 / 流派** 四维归类 | 与 FROST 电台 DJ 对话，**一句话自主成单**（按口味 + 场景 + 地理动态建歌单）、端侧排序选歌、云写串词（见 3.8） | 歌手出身地 / 歌曲城市（绿） |
...
> **当前路线**：现阶段先把 `music-curator`（音乐电台）作为核心 agent 跑通、做深——端侧自主成单 + 云端串词的完整闭环；`books` / `movies` / `podcast` 等其余 curator 已搭好双 Tab 骨架，后续按同一范式逐个深化。
